...
pub async fn codex_start(
    ...
    let proc = spawn_app_server(&bin).await.map_err(|e| e.to_string())?;
        ...
        tokio::spawn(async move {
            if let Some(mut child) = state_for_wait.take_child().await {
                ...
                let err = match exit {
                    ...
                    Ok(status) => Some(format!("codex exited: {status}")),
    ...
    let result = handshake(&client).await.map_err(|e| e.to_string())?;
