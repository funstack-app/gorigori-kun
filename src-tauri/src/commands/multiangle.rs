...
fn build_multiangle_prompt(cut: &CutPromptSpec, aspect_ratio: &str, environment: &str) -> String {
    ...
    format!(
        "添付したキャラクター参照画像を使い、次の構図で画像を1枚だけ生成してください。\n\n\
         ...
         4. image_gen ツールを1回だけ呼び出す。\n\
         5. アスペクト比は必ず {aspect_ratio} にする (縦長指定なら縦長で。勝手に 16:9 にしない)。high quality で生成する。\n\
         6. 画面内テキスト、ロゴ、透かし、グリッド、コラージュ、複数パネルは禁止。1枚の単独画像にする。\n\
         7. 出力先指定や画像変換は不要です。生成画像は $CODEX_HOME/generated_images/<session>/ig_*.png に保存されます。\n\n\
