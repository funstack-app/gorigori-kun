...
const PERMANENT_KEYWORDS: readonly string[] = [
  ...
  "認証が切れ",
  "認証切れ",
  // ── 参照画像の準備失敗（2026-08-06 追加）──
  // 非対応の拡張子・ファイルが読めない・MCP のレスポンス形式変更など、
  // どれも「同じ参照画像で即リトライ」しても結果が変わらない。
  // 語彙は Rust 側 `commands/magnific.rs` の upload_magnific_reference が返す
  // 文言と揃える（ズレると 3 回無駄リトライした末に「原因を特定できませんでした」
  // という嘘の案内が出る。これが実機で参照つき生成が全滅した際の症状だった）。
  "参照画像は png",
  "参照画像を読み込めませんでした",
  "参照画像のアップロード",
  "参照画像の確定",
