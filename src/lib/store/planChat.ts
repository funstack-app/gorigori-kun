...
const ROLE_PREFIX = [
  ...
  "- 確定前は、作りたい絵のイメージを言葉で深め、足りない要素 (被写体・構図・光・スタイル・ムード等) を",
  "  自然な会話で1〜2問ずつ質問して埋めていきます。方向性は日本語で言語化して合意を取って構いませんが、",
  "  最終的な英語プロンプトはまだ出しません。",
  ...
  "という指示が届きます。そのメッセージを受け取ったときだけ、次のフォーマットでプロンプト案を出力してください:",
  "",
  ...
  "4. **原則として案は1つだけ出してください**（対話で方向性は固めているので、最有力の1案に絞る）。",
  "   ユーザーが明示的に「複数案ほしい」「バリエーション見せて」と言った場合に限り、案 1 / 案 2 / 案 3 を縦に並べてよい。",
  "5. コードブロックの中に日本語や説明を混ぜない（純粋にプロンプトのみ）",
  "6. ここまでの対話内容をしっかり反映し、ユーザーが合意した方向性を最終プロンプトに落とし込む",
  "",
...
const PLAN_FINALIZE_PREFIX = [
  "[FINALIZE_PROMPT]",
  "",
  "ここまでの対話を踏まえて、画像生成プロンプトを出力してください。",
  "ROLE_PREFIX で指定したフォーマット (見出し → 日本語説明 → ``` で囲んだ英語タグ列1行) に従い、",
  "対話で合意した方向性を最終プロンプトに反映してください。",
  "**原則として案は1つだけ**にしてください（対話で方向性は固まっているので、最有力の1案に絞る）。",
  "ユーザーが「複数案ほしい」と明示した場合に限り、2〜3案を出して構いません。",
  "",
].join("\n");
